...
public class CosmosEnergyPickaxeItem extends PickaxeItem implements ICosmosEnergyItem {
	...
	public CosmosEnergyPickaxeItem(Tier itemTier, Properties builderIn, CosmosEnergyItem.Properties energyProperties) {
	...
	public boolean canAttackBlock(BlockState stateIn, Level worldIn, BlockPos posIn, Player playerEntity) {
		...
		if (heldStack.getItem() instanceof CosmosEnergyPickaxeItem) {
	...
	public int getMaxEnergyStored(ItemStack stackIn) {
		...
		return !(item instanceof CosmosEnergyPickaxeItem) ? 0 : ((CosmosEnergyPickaxeItem)item).maxEnergyStored;
	}

	@Override
	public int getMaxExtract(ItemStack stackIn) {
		Item item = stackIn.getItem();
		return !(item instanceof CosmosEnergyPickaxeItem) ? 0 : ((CosmosEnergyPickaxeItem)item).maxExtract;
	}

	@Override
	public int getMaxReceive(ItemStack stackIn) {
		Item item = stackIn.getItem();
		return !(item instanceof CosmosEnergyPickaxeItem) ? 0 : ((CosmosEnergyPickaxeItem)item).maxReceive;
	}

	@Override
	public int getMaxUse(ItemStack stackIn) {
		Item item = stackIn.getItem();
		return !(item instanceof CosmosEnergyPickaxeItem) ? 0 : ((CosmosEnergyPickaxeItem)item).maxUse;
	}

	@Override
	public boolean doesExtract(ItemStack stackIn) {
		Item item = stackIn.getItem();
		return !(item instanceof CosmosEnergyPickaxeItem) ? false : ((CosmosEnergyPickaxeItem)item).doesExtract;
	}

	@Override
	public boolean doesCharge(ItemStack stackIn) {
		Item item = stackIn.getItem();
		return !(item instanceof CosmosEnergyPickaxeItem) ? false : ((CosmosEnergyPickaxeItem)item).doesCharge;
	}

	@Override
	public boolean doesDisplayEnergyInTooltip(ItemStack stackIn) {
		Item item = stackIn.getItem();
		return !(item instanceof CosmosEnergyPickaxeItem) ? false : ((CosmosEnergyPickaxeItem)item).doesDisplayEnergyInTooltip;
	}
