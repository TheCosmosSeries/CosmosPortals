...
public class RendererPortalDockControllerSurface8 implements BlockEntityRenderer<BlockEntityPortalDockControllerSurface8> {
	...
	public RendererPortalDockControllerSurface8(BlockEntityRendererProvider.Context contextIn) {
		this.context = contextIn;
	}	

	@Override
	public void render(BlockEntityPortalDockControllerSurface8 entityIn, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int combinedLightIn, int combinedOverlayIn) {
		...
		if (level.getBlockState(entityPos).getBlock() instanceof BlockPortalDockControllerSurface8) {
			
			Direction facing = level.getBlockState(entityPos).getValue(BlockPortalDockControllerSurface8.FACING);
	
			int g = ComponentColour.BLACK.dec();
			int lg = ComponentColour.GRAY.dec();
			
			int cols[] = new int[] { 0, 0, 0, 0, 0, 0, 0, 0 };
			...
			if (entityIn.isLinked()) {
				...
				if (dockEntity != null) {
					...
					for (int i = 0; i < 8; i++) {
						cols[i] = dockEntity.getMaxSlotIndex() >= 0 ? (colours[i] == ComponentColour.EMPTY.dec() ? lg : colours[i]) : g;
					}
			...
			if (distanceToPlayer <= ModConfigManager.getInstance().getLabelMaximumDistance()) {
				...
				if (facing.equals(Direction.NORTH)) {
					...
					poseStack.translate(1 / 16D, 0.0D, 11 / 16D);
					poseStack.mulPose(Axis.YP.rotationDegrees(90));
					
					poseStack.pushPose();
					poseStack.translate(9 / 16D, 11 / 16D, 0 / 16D);
					buttonModel.renderToBuffer(poseStack, buttonBuilder, combinedLightIn, combinedOverlayIn, cols[0]);
					poseStack.popPose();
		
					poseStack.pushPose();
					poseStack.translate(9 / 16D, 11 / 16D, 5 / 16D);
					buttonModel.renderToBuffer(poseStack, buttonBuilder, combinedLightIn, combinedOverlayIn, cols[1]);
					poseStack.popPose();

					poseStack.pushPose();
					poseStack.translate(9 / 16D, 11 / 16D, 10 / 16D);
					buttonModel.renderToBuffer(poseStack, buttonBuilder, combinedLightIn, combinedOverlayIn, cols[2]);
					poseStack.popPose();
		

					poseStack.pushPose();
					poseStack.translate(9 / 16D, 6 / 16D, 0 / 16D);
					buttonModel.renderToBuffer(poseStack, buttonBuilder, combinedLightIn, combinedOverlayIn, cols[3]);
					poseStack.popPose();
		
					poseStack.pushPose();
					poseStack.translate(9 / 16D, 6 / 16D, 10 / 16D);
					buttonModel.renderToBuffer(poseStack, buttonBuilder, combinedLightIn, combinedOverlayIn, cols[4]);
					poseStack.popPose();
	

					poseStack.pushPose();
					poseStack.translate(9 / 16D, 1 / 16D, 0 / 16D);
					buttonModel.renderToBuffer(poseStack, buttonBuilder, combinedLightIn, combinedOverlayIn, cols[5]);
					poseStack.popPose();
					
					poseStack.pushPose();
					poseStack.translate(9 / 16D, 1 / 16D, 10 / 16D);
					buttonModel.renderToBuffer(poseStack, buttonBuilder, combinedLightIn, combinedOverlayIn, cols[7]);
					poseStack.popPose();
	
					poseStack.pushPose();
					poseStack.translate(9 / 16D, 1 / 16D, 5 / 16D);
					buttonModel.renderToBuffer(poseStack, buttonBuilder, combinedLightIn, combinedOverlayIn, cols[6]);
					poseStack.popPose();
				...
				if (facing.equals(Direction.SOUTH)) {
					...
					poseStack.translate(5 / 16.0D, 0.0D, -15 / 16D);
				...
				if (facing.equals(Direction.EAST)) {
					...
					poseStack.mulPose(Axis.YP.rotationDegrees(-90));
					poseStack.translate(5 / 16.0D, 0.0D, 1 / 16D);
				...
				if (facing.equals(Direction.WEST)) {
					...
					poseStack.mulPose(Axis.YP.rotationDegrees(-90));
					poseStack.translate(-11 / 16.0D, 0.0D, -15 / 16D);
				...
				if (ModConfigManager.getInstance().getRenderPortalLabels()) {
					if (distanceToPlayer <= ModConfigManager.getInstance().getLabelMaximumDistance()) {
						...
						MutableComponent text[] = new MutableComponent[] { ComponentHelper.empty(), ComponentHelper.empty(), ComponentHelper.empty(), ComponentHelper.empty(), ComponentHelper.empty(), ComponentHelper.empty(), ComponentHelper.empty(), ComponentHelper.empty() };
						...
						if (facing.equals(Direction.SOUTH)) {
							...
							poseStack.translate(8 / 16F, 8 / 16F, 23 / 16F);
							
							poseStack.pushPose();
							poseStack.translate(9.2 / 16F, 10.75 / 16F, -9 / 16F);
							CosmosRendererHelper.renderLabelInWorld(fontRenderer, poseStack, text[0], bufferIn, combinedLightIn, false, currentIndex == 0);
							poseStack.popPose();
							
							poseStack.pushPose();
							poseStack.translate(2.45 / 16F, 10.75 / 16F, -9 / 16F);
							CosmosRendererHelper.renderLabelInWorld(fontRenderer, poseStack, text[1], bufferIn, combinedLightIn, false, currentIndex == 1);
							poseStack.popPose();
							
							poseStack.pushPose();
							poseStack.translate(-4.2 / 16F, 10.75 / 16F, -9 / 16F);
							CosmosRendererHelper.renderLabelInWorld(fontRenderer, poseStack, text[2], bufferIn, combinedLightIn, false, currentIndex == 2);
							poseStack.popPose();
							
							
							poseStack.pushPose();
							poseStack.translate(9.25 / 16F, 4.1 / 16F, -9 / 16F);
							CosmosRendererHelper.renderLabelInWorld(fontRenderer, poseStack, text[3], bufferIn, combinedLightIn, false, currentIndex == 3);
							poseStack.popPose();
	
							poseStack.pushPose();
							poseStack.translate(-4.2 / 16F, 4.1 / 16F, -9 / 16F);
							CosmosRendererHelper.renderLabelInWorld(fontRenderer, poseStack, text[4], bufferIn, combinedLightIn, false, currentIndex == 4);
							poseStack.popPose();
	
							
							poseStack.pushPose();
							poseStack.translate(9.2 / 16F, -2.8 / 16F, -9 / 16F);
							CosmosRendererHelper.renderLabelInWorld(fontRenderer, poseStack, text[5], bufferIn, combinedLightIn, false, currentIndex == 5);
							poseStack.popPose();
							
							poseStack.pushPose();
							poseStack.translate(2.45 / 16F, -2.8 / 16F, -9 / 16F);
							CosmosRendererHelper.renderLabelInWorld(fontRenderer, poseStack, text[6], bufferIn, combinedLightIn, false, currentIndex == 6);
							poseStack.popPose();
							
							poseStack.pushPose();
							poseStack.translate(-4.2 / 16F, -2.8 / 16F, -9 / 16F);
							CosmosRendererHelper.renderLabelInWorld(fontRenderer, poseStack, text[7], bufferIn, combinedLightIn, false, currentIndex == 7);
							poseStack.popPose();
						...
						if (facing.equals(Direction.NORTH)) {
							...
							poseStack.translate(-13.4 / 16F, 8 / 16F, 7 / 16F);
						...
						if (facing.equals(Direction.WEST)) {
							...
							poseStack.translate(8 / 16F, 8 / 16F, 7 / 16F);
						...
						if (facing.equals(Direction.EAST)) {
							...
							poseStack.translate(-13.25 / 16F, 8 / 16F, 23 / 16F);
	...
	public class ButtonModel extends Model {
		...
		public static LayerDefinition createLayer() {
			...
			partdefinition.addOrReplaceChild(STRING, CubeListBuilder.create().texOffs(0, 0).addBox(0.0F, 0.0F, 0.0F, 1.0F, 4.0F, 4.0F), PartPose.ZERO);
