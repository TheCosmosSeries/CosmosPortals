...
@OnlyIn(Dist.CLIENT)
public class RendererPortalDockController8 implements BlockEntityRenderer<BlockEntityPortalDockController8> {
	...
	public RendererPortalDockController8(BlockEntityRendererProvider.Context contextIn) {
		this.context = contextIn;
	}	

	@Override
	public void render(BlockEntityPortalDockController8 entityIn, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int combinedLightIn, int combinedOverlayIn) {
			...
			if (ModConfigManager.getInstance().getRenderPortalLabels()) {
				if (distanceToPlayer <= ModConfigManager.getInstance().getLabelMaximumDistance()) {
					...
					if (south.isAir()) {
						...
						poseStack.translate(-13.4 / 16F, 8 / 16F, -8 / 16F);
					...
					if (west.isAir()) {
						...
						poseStack.translate(-13.25 / 16F, 8 / 16F, 8 / 16F);
